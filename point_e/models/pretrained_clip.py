...
from point_e.models.download import default_cache_dir
...
class ImageCLIP(nn.Module):
    ...
    A wrapper module around a pretrained OpenAI CLIP model, enabling flexible, multimodal embedding of images, text,
    and precomputed embeddings in a unified interface. It's designed for use in downstream models, such as generative
    pipelines like those in Point-E, where you may have to handle inputs from different modalities and batch them
    efficiently
    """

    def __init__(self, device: torch.device, dtype: Optional[torch.dtype] = torch.float32,
                 ensure_used_params: bool = True, clip_name: str = "ViT-L/14", cache_dir: Optional[str] = None, ):
    ...
    def _static_multimodal_embed(self, images: List[Optional[ImageType]] = None,
                                 texts: List[Optional[str]] = None,
                                 embeddings: List[Optional[torch.Tensor]] = None, ) -> torch.Tensor:
        """
        ...
        joined_embs = torch.stack([emb.to(device=self.device, dtype=torch.float32)
                                   if emb is not None else torch.zeros(self.feature_dim, device=self.device)
                                   for emb in embeddings], dim=0, )
        ...
        return (image_flag.float() * image_emb + text_flag.float() * text_emb + emb_flag.float() * joined_embs
    ...
    def embed_images_grid(self, xs: Iterable[Optional[ImageType]]) -> torch.Tensor:
        ...
        x = torch.cat([vt.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1],
    ...
    def forward(self, batch_size: int, images: Optional[Iterable[Optional[ImageType]]] = None,
                texts: Optional[Iterable[Optional[str]]] = None,
                embeddings: Optional[Iterable[Optional[torch.Tensor]]] = None, ) -> torch.Tensor:
        """
        ...
        if self.ensure_used_params:
            # To ensure that all model parameters are "used" in the computation graph, even if some branches
            # (text/image/embed) are unused in a particular forward pass.
        ...
        for i, (image, text, emb) in enumerate(zip(image_seq, text_seq, embedding_seq)):
            assert (sum([int(image is not None), int(text is not None), int(emb is not None)]) < 2), \
...
class FrozenImageCLIP:
    ...
    def __call__(self, batch_size: int, images: Optional[Iterable[Optional[ImageType]]] = None,
                 texts: Optional[Iterable[Optional[str]]] = None,
                 embeddings: Optional[Iterable[Optional[torch.Tensor]]] = None) -> torch.Tensor:
