...
from point_e.diffusion.gaussian_diffusion import GaussianDiffusion, get_named_beta_schedule, SpacedDiffusion, \
    space_timesteps
...
def diffusion_from_config(config: Dict[str, Any]) -> GaussianDiffusion:
    ...
    respace = config.get("respace", None)
    ...
    kwargs = dict(betas=betas, model_mean_type=mean_type, model_var_type="learned_range", loss_type="mse",
                  channel_scales=channel_scales, channel_biases=channel_biases,)
